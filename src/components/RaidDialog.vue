  ...
  <q-dialog v-model="model" persistent>
    <q-card dark class="bg-blue-grey-9">
      <q-card-section class="row items-center bg-blue-grey-8 text-white">
        ...
        <span class="q-ml-sm">{{ raid?.name|| 'Add new' }}</span>
      </q-card-section>
      <q-card-section>
        <RaidForm ref="raidFormRef" :data="raid" :case_id="caseId" />
      </q-card-section>
      <q-card-actions align="right">
        <q-btn v-if="raid?.id" no-caps="" label="Delete" color="red" v-close-popup @click="onDelete" />
        <q-space />
        <q-btn no-caps="" label="Submit" color="primary" v-close-popup @click="onSubmit" />
        <q-btn no-caps="" label="Cancel" color="primary" v-close-popup @click="onCancel" />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
