...
export async function getDateData(date, month, year, collectionId) {
  ...
  const snapshot = await getDocsFromServer(q); //getCountFromServer(q);
  if (snapshot.empty) return 0;
  const data = [];
  snapshot.forEach(async (s) => {
    //console.log(s, s.data());
    await data.push(s.data().Amount);
  });
  return data.reduce((d, v) => d + v);
}
...
export async function getQuarterData(quater, year, collectionId) {
  ...
  return data.reduce((d, v) => Number(d) + v);
}
...
export async function getQuarterlyData(year, collectionId) {
  ...
  //console.log(data);
  return { data, total: data.reduce((d, v) => Number(d) + v) };
}
